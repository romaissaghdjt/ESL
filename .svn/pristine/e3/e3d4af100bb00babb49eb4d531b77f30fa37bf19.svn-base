...
- (void)request
{
    NSString*url = [WeddURL stringByAppendingString:@"weddingHandler.ashx?Action=PersonalTailor"];
    NSLog(@"%@",url);
    [zJRequestManager requestWithUrl:url parmDic:nil method:GET1 succeedWithData:^(NSData *data) {
        ...
        for (NSDictionary*dic in ary) {
            ...
            self.imgArray = [NSMutableArray arrayWithArray:model.Img];
        }
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        [cell.displayView sd_setImageWithURL:[NSURL URLWithString:_imgArray[indexPath.row]]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        if ([self.str isEqualToString:@"私人订制"]) {
            ...
            WeddingModel*model = _array[indexPath.row];
