...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    {
        NSString*ss = [[NSUserDefaults standardUserDefaults]objectForKey:@"us"];
        self.PhoneTextF.text = ss;
...
- (IBAction)ChooseQuClick:(id)sender {
    ...
    [self request: [WeddURL stringByAppendingString: @"HallsHandler.ashx?Action=getCounty&CityID=1"] str:@"区"];
...
- (IBAction)ChooseCXClick:(id)sender {
    ...
    self.TuanDuiButton.enabled = NO;
    self.areaArray = [NSMutableArray array];
    [self request:[WeddURL stringByAppendingString: @"BookHandler.ashx?Action=bookPage"] str:@"series"];
    self.dropdownMenu = [[UIImageView alloc]init];
    self.dropdownMenu.userInteractionEnabled = YES;//开启交互功能
    self.dropdownMenu.image = [UIImage imageNamed:@"喜事堂_03(2).png"];
    self.dropdownMenu.frame = CGRectMake(0,30, KWIDTH,KHEIGHT);
    self.dropdownMenu.backgroundColor = [UIColor whiteColor];
    
    self.DuoXbutton = [[UIButton alloc]initWithFrame:CGRectMake(0, KHEIGHT-72*SWIDTH_SCALE, KWIDTH, 50*SHEIGHT_DCALE)];
    ...
    tableView.frame = CGRectMake(0, 30, KWIDTH, KHEIGHT-100);
...
- (IBAction)JDClick:(id)sender {
    ...
    [self.dropdownMenu removeFromSuperview];
    self.areaArray = [NSMutableArray array];
    NSString*url =[WeddURL stringByAppendingString: @"JoinHandler.ashx?Action=GetMaxOrderNums"];
    //NSLog(@"%@",url);
...
- (IBAction)NextClick:(id)sender {
    ...
    ToJoinVC.str = self.TuanDOrGLable.text;
    ToJoinVC.Fanwei = self.ID;
    ToJoinVC.name = self.NameTextF.text;
    ToJoinVC.tel = self.PhoneTextF.text;
    NSString*str = @"";
    for (int i = 0; i < _IDArr.count; i++) {
        str = [NSString stringWithFormat:@"%@H",[str stringByAppendingString:_IDArr[i]]];
    }
    NSLog(@"%@",str);
    ToJoinVC.caiXI = str;
    ToJoinVC.power = self.ID;
...
- (void)creatView:(NSString*)str view:(UIView*)view
{
    ...
    self.dropdownMenu.userInteractionEnabled = YES;//开启交互功能
    self.dropdownMenu.image = [UIImage imageNamed:@"喜事堂_03(2).png"];
    self.dropdownMenu.frame = CGRectMake(100*SWIDTH_SCALE,view.frame.origin.y+95, 260*SWIDTH_SCALE, 200*SHEIGHT_DCALE);
    self.dropdownMenu.backgroundColor = [UIColor whiteColor];
    self.dropdownMenu.layer.cornerRadius= 15;
    self.dropdownMenu.layer.masksToBounds = YES;
    self.dropdownMenu.layer.borderWidth = 2;
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_i == 0) {
        ...
        cell.textLabel.text = model.CountyName;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
     if (_i == 0) {
         ...
         self.JDButton.enabled = YES;
        ToJionInModel*model = self.areaArray[indexPath.row];
        self.str = model.CountyName;
        self.QuLable.text = self.str;
        self.ID = model.CountyID;
        [self.dropdownMenu removeFromSuperview];
    }else if(_i == 1){
