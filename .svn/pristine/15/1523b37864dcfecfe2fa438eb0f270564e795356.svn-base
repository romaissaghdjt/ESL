...
- (void)viewDidLoad {
    ...
    self.isFail = NO;
    for (int i = 0 ; i < 5; i++) {
        UIImageView*imageView = [[UIImageView alloc]initWithFrame:CGRectMake(10+i*60, self.backView.bottom+10, 60, 60)];
        imageView.tag = 100+i;
...
- (void)request
{
    NSString*str = [PREURL stringByAppendingString: [NSString stringWithFormat:@"ServerManager.ashx?ServerModule=CookServer&Function=HttpJoin&UserTel=%@&UserPhyAdd=%@&GoodAt=%@&MaxRecvOrdAbility=%@&ServiceArea=%@&IDNum=%@",us,KUUID,self.caiXI,self.power,self.Fanwei,self.IDTextF.text]];
    ...
    [manager POST:string parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        }else if ([str isEqualToString:@"申请成功"])
        {
            [self showSmessage:@"加盟成功"];
        }else{
            NSString*reson = responseObject[@"Reason"];
            [self showSmessage:reson];
        }
...
//- (void)requestT
//{
//    NSString*str = [PREURL stringByAppendingString:[NSString stringWithFormat:@"JoinHandler.ashx?Action=CookJoin&CookOrGrp=Grp&Name=%@&Tel=%@&GoodAt=%@&Maximum=%@&ServiceArea=%@&IDNum=%@&ApplyTime=%@",self.name,self.tel,self.caiXI,self.power,self.Fanwei,self.IDTextF.text,self.SetUpDt]];///////
//    NSLog(@"%@",str);
//    NSString *string = [str stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
//    NSLog(@"%@",string);
//    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
