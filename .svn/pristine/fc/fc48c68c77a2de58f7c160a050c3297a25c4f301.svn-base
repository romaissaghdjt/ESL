...
@interface OrderCalendarView()<UIScrollViewDelegate>
//中间日期数组
...
@implementation OrderCalendarView
...
-(NSString *)getDateString:(NSString *)dateString Type:(TYPE)type Date:(NSDate *)date
{
    ...
    switch (type) {
        case PRE:
            startMonthStr = (tempMonthStr - 1) > 0 ? [NSString stringWithFormat:@"%02d", (tempMonthStr - 1)] : @"12";
            startYearStr = [startMonthStr isEqualToString:@"12"] ? [NSString stringWithFormat:@"%d", ([NSDate year:date] - 1)] : [NSString stringWithFormat:@"%ld", (long)[NSDate year:date]];
            break;
        case NEX:
            startMonthStr = (tempMonthStr + 1) > 12 ?  @"1": [NSString stringWithFormat:@"%02d", (tempMonthStr + 1)];
            startYearStr = [startMonthStr isEqualToString:@"1"] ? [NSString stringWithFormat:@"%d", ([NSDate year:date] + 1)] : [NSString stringWithFormat:@"%ld", (long)[NSDate year:date]];
            break;
        default:
...
{
    NSLog(@"%ld", [NSDate year:[NSDate date]]*10000+[NSDate month:[NSDate date]]*100+[NSDate day:[NSDate date]]);
    if ([[[self getChooseDate:sender] stringByReplacingOccurrencesOfString:@"/" withString:@""] integerValue] - ([NSDate year:[NSDate dateWithTimeIntervalSinceNow:7*60*60*24]]*10000+[NSDate month:[NSDate dateWithTimeIntervalSinceNow:7*60*60*24]]*100+[NSDate day:[NSDate dateWithTimeIntervalSinceNow:7*60*60*24]]) < 0)
    {
        [UIViewController showMessage:@"请提前一周预订" ViewController:[self getCurrentVC]];
        return;
    }
